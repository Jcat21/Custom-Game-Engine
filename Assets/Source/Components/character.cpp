...
/*static*/ void Character::ProcessAnim(Character* pCharacter)
{
    std::vector<Matrix4> pose;
    pCharacter->mCurrentAnim->GetGlobalPoseAtTime(pose, pCharacter->mSkeleton, pCharacter->mAnimationTime);
    const std::vector<Matrix4>& invBind = pCharacter->mSkeleton->GetGlobalInvBindPoses();
    for (size_t i = 0; i < pose.size(); ++i)
    {
        pCharacter->mSkinnedObj->mSkinnedData.matrixPalette[i] = invBind[i] * pose[i];
    }
}


Character::AnimJob::AnimJob(Character* pCharacter)
...
{
    Character::ProcessAnim(mCharacter);
}