...
inline Vector3 TransformWithPerspDiv(const Vector3& vec, const Matrix4& mat, float w = 1.0f)
{
    ...
    if (!Math::IsZero(fabsf(transformedW)))
    {
...
// This will transform the vector and renormalize the w component
inline Vector4 TransformWithPerspDiv(const Vector4& vec, const Matrix4& mat)
{
    Vector4 retVal = Transform(vec, mat);
    if (!Math::IsZero(fabsf(retVal.w)))
    {
        float invW = 1.0f / retVal.w;
        retVal *= invW;
    }
    return retVal;
}
