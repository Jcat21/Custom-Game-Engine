...
void Game::Init(HWND hWnd, float width, float height)
{
	...
	{   // load the Normal shader
		D3D11_INPUT_ELEMENT_DESC meshLayout[] =
		{
			...
			{ "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(VertexNormal, norm), D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "Tangent", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, offsetof(VertexNormal, tangent), D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, offsetof(VertexNormal, uv), D3D11_INPUT_PER_VERTEX_DATA, 0 },
		};
		Shader* pShader = new Shader(&mGraphics);
		pShader->Load(L"Shaders/Normal.hlsl", meshLayout, 4);
...
#else
    LoadLevel(L"Assets/Levels/Level09.itplevel");
#endif
            ...
            if (components.IsArray())
            {
                for (rapidjson::SizeType componentIndex = 0; componentIndex < components.Size(); ++componentIndex)
                {
					...
					else if (compType == "Player")
					{
						if (nullptr != pSkinObj)
						{
							Player* pPlayer = new Player(pSkinObj);
							pPlayer->LoadProperties(components[componentIndex]);
							pObj->AddComponent(pPlayer);
						}
					}
					else if (compType == "FollowCam")
					{
						FollowCam* followCam = new FollowCam(pObj);
						followCam->LoadProperties(components[componentIndex]);
						pObj->AddComponent(followCam);
					}
					else if (compType == "Collision")
					{
						CollisionComponent* collisionComp = new CollisionComponent(pObj);
						collisionComp->LoadProperties(components[componentIndex]);
						pObj->AddComponent(collisionComp);
						mPhysics.AddObj(collisionComp);
					}
					else if (compType == "SimpleRotate")
					{
						SimpleRotate* rotationComp = new SimpleRotate(pObj);
						rotationComp->LoadProperties(components[componentIndex]);
						pObj->AddComponent(rotationComp);
					}
#endif
				}
            }
